...
def _make_hedft(t_ref: np.ndarray, v1z: np.ndarray, v2z: np.ndarray,
    ...
    return HedftTrajectory(
        ...
        v1_y_Aps=np.zeros_like(v1z),
        v2_y_Aps=np.zeros_like(v2z),
        distance_A=np.linspace(2.0, 5.0, t_ref.size),
        x1_A=np.zeros_like(v1z),
        x2_A=np.zeros_like(v2z),
        y1_A=np.zeros_like(v1z),
        y2_A=np.zeros_like(v2z),
        z1_A=np.zeros_like(v1z),
        z2_A=np.zeros_like(v2z),
